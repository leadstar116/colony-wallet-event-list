...
const UserList = ({ searchString }: Props) => {
  ...
  // Preload users
  useEffect(() => {
    if (userList.isPreloaded) return;
    dispatch(loadUsers(usersCount));
  }, [dispatch, userList, usersCount]);

  // Initialize users at first load
  useEffect(() => {
    if (!userList.isPreloaded || isInitialized || userList.users.length) return;
  ...
  const filtereUsersWithSearchKey = () => {
    return userList.users.filter((user) => {
      ...
      return name
        ...
        .join("")
        .toLowerCase()
        .includes(searchString.split(" ").join("").toLowerCase());
    });
  };
